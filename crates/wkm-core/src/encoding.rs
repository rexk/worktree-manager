...
/// Validate a worktree alias. Aliases are:
...
/// alias map and does not shadow the built-in `@main` token.
pub fn validate_alias(alias: &str) -> Result<(), String> {
    ...
    if alias.len() > MAX_ALIAS_LENGTH {
        return Err(format!(
            "alias '{alias}' is too long (max {MAX_ALIAS_LENGTH} chars)"
        ));
    }
    if alias.starts_with('@') {
        return Err(format!(
            "alias '{alias}' cannot start with '@' (reserved for built-in tokens)"
        ));
    }
    ...
    for c in chars {
        if !(c.is_ascii_lowercase() || c.is_ascii_digit() || c == '_' || c == '-') {
            return Err(format!(
                "alias '{alias}' contains invalid character '{c}' (allowed: a-z 0-9 _ -)"
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    fn validate_alias_accepts_valid_names() {
        assert!(validate_alias("specs").is_ok());
        assert!(validate_alias("bugfix-3").is_ok());
        assert!(validate_alias("feat_a").is_ok());
        assert!(validate_alias("9lives").is_ok());
    ...
    fn validate_alias_rejects_at_prefix() {
        assert!(validate_alias("@main").is_err());
        assert!(validate_alias("@specs").is_err());
    }
    ...
    fn validate_alias_rejects_too_long() {
        ...
        assert!(validate_alias(&exact).is_ok());
    }
}
